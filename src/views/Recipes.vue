...
const filteredRecipes = computed(() => {
  return recipes.value.filter((recipe) => {
    ...
    if (
      ...
      !recipe.name
        .toLowerCase()
        .includes(searchCriteria.value.title.toLowerCase())
    ) {
      return false
    }
    if (
      searchCriteria.value.courses.length > 0 &&
      !searchCriteria.value.courses.includes(recipe.course.toLowerCase())
    ) {
      return false
    }
    if (
      searchCriteria.value.difficulties.length > 0 &&
      !searchCriteria.value.difficulties.includes(
    ...
    if (searchCriteria.value.minTime || searchCriteria.value.maxTime) {
      ...
      const minTime = searchCriteria.value.minTime
        ? getTimeInMinutes(searchCriteria.value.minTime)
        : 0
      const maxTime = searchCriteria.value.maxTime
        ? getTimeInMinutes(searchCriteria.value.maxTime)
        : Infinity
    ...
    // Ingredient filters
    if (
      searchCriteria.value.includedIngredients.length > 0 &&
      recipe.ingredients
    ) {
      const hasAllIngredients = searchCriteria.value.includedIngredients.every(
        (ingredient) =>
    ...
    if (
      searchCriteria.value.excludedIngredients.length > 0 &&
      recipe.ingredients
    ) {
      const hasExcludedIngredients =
        searchCriteria.value.excludedIngredients.some((ingredient) =>
          recipe.ingredients?.some((recipeIngredient) =>
            recipeIngredient.toLowerCase().includes(ingredient.toLowerCase())
          )
        )
...
function handleSearch(criteria: any) {
  searchCriteria.value = {
    ...
    difficulties: criteria.difficulties.map((difficulty: string) =>
...
function getTimeInMinutes(timeString: string): number {
  ...
  const hours = timeString.includes('hour')
    ? parseInt(timeString.split(' ')[0])
    : 0
  const minutes = timeString.includes('minutes')
    ? parseInt(timeString.split(' ')[0])
    : 0
